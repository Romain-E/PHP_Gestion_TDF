...
if(isset($nom_sponsor) && isset($nom_abr_sponsor) && isset($_POST['spo_nat']) && isset($annee_sponsor) && isset($_POST['spo_equ'])){
	
	
	$nat_sponsor = $_POST['spo_nat'];
	$num_equipe_sponsor = $_POST['spo_equ'];
	...
	if($testReg[0]['NOM'] != $nom_sponsor || $testReg[0]['NA_SPONSOR'] != $nom_abr_sponsor || $testReg[0]['CODE_CIO'] != $_POST['spo_nat']) {
		if($testReg[0]['ANNEE_SPONSOR'] < $annee_sponsor) {
			...
			require_once ("creer_sponsor.html");
		}
	}
	else
		require_once ("creer_sponsor.html");	
