<?php
  ...
  if(isset($_POST['cou_nom_recherche']) && $_POST['cou_nom_recherche']!=""){
	$_POST['cou_nom_recherche'] = preg_replace("/'/","''",$_POST['cou_nom_recherche']);
...
if(isset($_POST['triage'])) {
  ...
  if(isset($_POST['actual_num_cour_suppr'])){
      ...
      $req = "SELECT cou.N_COUREUR, cou.NOM as NOM_COUREUR, PRENOM, ANNEE_NAISSANCE, ANNEE_PREM, pays.NOM as NOM_PAYS
        ...
        WHERE cou.N_COUREUR =".$_POST['actual_num_cour_suppr'];
      ...
      if (empty($tab_suppr[0]["ANNEE_PREM"])){
          ...
          <p style="text-align:center;">Voulez-vous vraiment supprimer le coureur '. $_POST['actual_num_cour_suppr'] .' ? </br>
          Nom : '.$tab_suppr[0]["NOM_COUREUR"].'<br>
          Prénom : '.$tab_suppr[0]["PRENOM"].'<br>
          Nationalité : '.$tab_suppr[0]["NOM_PAYS"].'<br></p>
          <input type="hidden" name="cou_num_suppr" value="' . $_POST['actual_num_cour_suppr'] . '">
          ...
          echo '<p style="text-align:center;">Le coureur '. $_POST['actual_num_cour_suppr'] .' ne peut être supprimé car il a <b>déjà participé</b> à un tour de France. </br>
          Nom : '.$tab_suppr[0]["NOM_COUREUR"].'<br>
          Prénom : '.$tab_suppr[0]["PRENOM"].'<br>
  ...
  if($nb_ligne > 0){
    ...
    for ($i=0;$i<$nb_ligne;$i++) {
      ...
      <input type="hidden" name="actual_num_cour_suppr" value='. $tab[$i]["N_COUREUR"] .'>
      <td align="center"><button onclick="submit">✖</button></td>
      </form>

      <form action="consulter_coureur.php" method="post" enctype="application/x-www-form-urlencoded">
      <input type="hidden" name="actual_num_cour_m" value='. $tab[$i]["N_COUREUR"] .'>
      <td align="center"><button onclick="submit">🖉</button></td>
      </form>
